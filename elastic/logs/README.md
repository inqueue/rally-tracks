...
* `start_date` (default: _current_time_ in `%Y-%m-%d` format) - The start date of the data. The `end_date` minus this value will determine the time range assigned to the data and also directly impact the total volume indexed. Must be less than the `end_date`. Leave unset to use the current time.
* `end_date` (default: `1`) - duration in days or date formatted string indicating the end of the data. When specified as an integer, calculate the end date as a duration in days from the `start_date`. This value minus the `start_date` will determine the time range assigned to the data and also directly impact the total volume indexed. Must be greater than the `start_date`.
